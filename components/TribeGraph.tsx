    ...
    <div className='relative rounded-2xl border border-stone-200 bg-white shadow-sm'>
      ...
      >
        ...
        <g ref={gRef}>
          ...
          {links.map((l, idx) => {
            ...
            if (!a || !b) return null;
            const stroke =
              l.kind === 'war' ? COLORS.edge.war : COLORS.edge.alliance;
            const dash = l.kind === 'war' ? '6,4' : undefined;
            const marker =
              l.kind === 'war' ? 'url(#arrowWar)' : 'url(#arrowAlliance)';
            return (
              <g key={idx}>
                <line
                  x1={X(a)}
      ...
      <div className='absolute left-3 top-3 rounded-md bg-white/90 p-2 text-xs shadow ring-1 ring-stone-200'>
        ...
        <div className='flex flex-wrap gap-x-3 gap-y-1'>
          ...
          <LegendLine
            color={COLORS.edge.war}
            label='War/Feud (pre-Islamic)'
            dash
          />
        </div>
      </div>
        ...
        >
          ...
          {tooltip.node.chiefs?.length ? (
            <div className='mt-1 text-stone-700'>
              <span className='font-medium'>Chiefs:</span>{' '}
              {tooltip.node.chiefs.join(', ')}
            </div>
          ) : null}
          {tooltip.node.elders?.length ? (
            <div className='mt-1 text-stone-700'>
              <span className='font-medium'>Elders/Notables:</span>{' '}
              {tooltip.node.elders.join(', ')}
            </div>
          ) : null}
      ...
      {selected && (
        <div className='border-t border-stone-200 p-3 text-sm'>
          ...
          {selected.chiefs?.length ? (
            <p className='mt-2'>Chiefs: {selected.chiefs.join(', ')}</p>
          ) : null}
          {selected.elders?.length ? (
            <p className='mt-1'>
              Elders/Notables: {selected.elders.join(', ')}
            </p>
          ) : null}
    ...
    <span className='inline-flex items-center gap-1'>
      <span
        ...
        style={{
          ...
          borderTop: dash ? '1px dashed ' + color : undefined,
        }}
      />
      <span className='text-stone-700'>{label}</span>
    </span>
