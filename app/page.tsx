...
import { fetchEventCards } from '@/lib/api/public';
...
export default function LandingPage() {
  ...
  useEffect(() => {
    fetchEventCards()
      .then(events => setEventCount(events.length))
    ...
    <main className='space-y-8'>
        ...
        <div className='relative z-10'>
          <div className='fade-in-up'>
            ...
            <p className='text-lg md:text-xl text-stone-700 max-w-3xl leading-relaxed mb-8'>
              Explore the life of Prophet Muhammad (ﷺ) through interactive timelines, immersive stories, and timeless lessons.
            </p>
            <div className='flex flex-col sm:flex-row gap-4'>
              <Link href='/importance' className='inline-flex items-center px-6 py-3 bg-stone-900 text-white rounded-xl hover:bg-stone-800 transition-colors duration-300 font-medium'>
                Why Study Seerah?
              </Link>
              <Link href='#timeline' className='inline-flex items-center px-6 py-3 border-2 border-stone-300 text-stone-700 rounded-xl hover:border-stone-400 hover:bg-white/50 transition-all duration-300 font-medium'>
                Explore Timeline
              </Link>
            </div>
          </div>
      ...
      <section className='grid grid-cols-1 md:grid-cols-3 gap-6'>
        <div className='card p-6 text-center fade-in-up stagger-1'>
          <div className='text-3xl font-bold text-amber-600 mb-2'>{eventCount}</div>
      ...
      <section id='timeline' className='fade-in-up stagger-4'>
        <div className='flex items-center justify-between mb-8'>
          <div>
            ...
            <p className='text-stone-600 max-w-2xl'>Journey through pivotal moments in Islamic history.</p>
          </div>
        </div>
        <Timeline />
      </section>
