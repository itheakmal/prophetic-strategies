...
export default function EventPage({ params }: EventPageProps) {
  ...
  return (
    <div className="space-y-8">
      ...
      <Section title="Choose Your Path" subtitle="Was this an action or a reaction?">
      ...
      {state.choice && (
        <Section title="Deep Reflection" subtitle={state.choice === 'action' ? 'Why did he take this action?' : 'How did he respond?'}>
          <div className="space-y-6">
            <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-stone-50 to-stone-100/50 p-6">
              <p className="text-lg font-medium text-stone-800 mb-6">{event.followups[state.choice].prompt}</p>

              {(event.followups[state.choice].type === 'mcq') && (
                <div className="grid gap-4 md:grid-cols-2">
                  {event.followups[state.choice].choices.map((c:string, idx:number) => (
                    <label key={idx} className={`group flex cursor-pointer items-start gap-4 rounded-xl border-2 p-4 transition-all duration-300 ${
                      ...
                      <input type="radio" name="followup" className="mt-1 w-5 h-5 text-amber-600" checked={state.answerIndex === idx} onChange={() => setAnswerIndex(idx)} />
                      <span className="text-stone-800 font-medium group-hover:text-amber-700 transition-colors duration-300">{c}</span>
                    </label>
                  ))}
                </div>
              )}
              ...
              {(event.followups[state.choice].type === 'dropdown') && (
                <div className="max-w-md">
                  <select className="w-full rounded-xl border-2 border-stone-300 bg-white p-4 text-stone-800 focus:border-amber-500 focus:outline-none transition-colors duration-300" value={state.answerIndex ?? ''} onChange={(e) => setAnswerIndex(Number(e.target.value))}>
              ...
              <div className="flex flex-wrap gap-3">
                <button onClick={() => setAnswerChecked(true)} disabled={state.answerIndex === null} className="rounded-xl bg-stone-900 px-6 py-3 text-sm font-bold text-white disabled:cursor-not-allowed disabled:opacity-50 hover:bg-stone-800 transition-colors duration-300">
                  Check Answer
                </button>
                <button onClick={() => setThinkDeeper(!state.thinkDeeper)} className="rounded-xl border-2 border-stone-300 bg-white px-6 py-3 text-sm font-bold text-stone-800 hover:bg-stone-100 hover:border-amber-300 transition-all duration-300">
            ...
            {state.answerChecked && (
              <div className={`rounded-2xl p-6 text-sm border-2 ${
                isAnswerCorrect() 
                ...
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <p className="font-bold text-lg">{isAnswerCorrect() ? 'Correct! ✔' : "Let's revisit ✧"}</p>
                </div>
                <p className="leading-6">{event.followups[state.choice].explanation}</p>
              </div>
            )}
            ...
            {state.thinkDeeper && (
              <div className="rounded-2xl border-2 border-stone-200 bg-gradient-to-br from-stone-50 to-stone-100/50 p-6">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <h4 className="font-bold text-lg text-stone-800">Scholarly Interpretations</h4>
                </div>
                <ul className="space-y-3 text-stone-700">
                  <li className="flex items-start gap-3">
                    <div className="w-2 h-2 bg-purple-500 rounded-full mt-2 flex-shrink-0"></div>
                    <span>Compare civil pacts vs. state law: how prophetic endorsement shapes civic virtue.</span>
                  </li>
                  <li className="flex items-start gap-3">
                    <div className="w-2 h-2 bg-purple-500 rounded-full mt-2 flex-shrink-0"></div>
                    <span>Map to maqāṣid al-sharīʿah and prophetic values (ʿadl/justice, ḥifẓ al-ḥuqūq/rights).</span>
                  </li>
                  <li className="flex items-start gap-3">
                    <div className="w-2 h-2 bg-purple-500 rounded-full mt-2 flex-shrink-0"></div>
                    <span>Insert authenticated citations from your Sessions corpus here.</span>
                  </li>
                </ul>
              </div>
            )}
      ...
      <Section title="Wisdom & Application" subtitle="Lessons, modern parallels, and your personal reflection">
          ...
          <div className="space-y-6">
            <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-emerald-50 to-emerald-100/50 p-6">
              <div className="flex items-center gap-3 mb-4">
                ...
                <h4 className="text-xl font-serif font-bold text-emerald-900">Key Lessons</h4>
              </div>
            ...
            <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-blue-50 to-blue-100/50 p-6">
              <div className="flex items-center gap-3 mb-4">
                ...
                <h4 className="text-xl font-serif font-bold text-blue-900">Modern Parallels</h4>
              </div>
              <ul className="space-y-3 max-h-64 overflow-auto">
          ...
          <div>
            <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-amber-50 to-amber-100/50 p-6">
              <div className="flex items-center gap-3 mb-4">
                ...
                <h4 className="text-xl font-serif font-bold text-amber-900">Personal Reflection</h4>
              </div>
              <label htmlFor="journal" className="block text-sm font-medium text-amber-800 mb-3">How can you apply this today?</label>
              <textarea 
                id="journal" 
                value={state.journal} 
              ...
              <div className="mt-3 flex items-center justify-between text-xs text-amber-700">
                <span className="font-medium">{state.journal.length} characters</span>
                {state.savedAt && <span className="font-medium">Saved: {state.savedAt}</span>}
              </div>
              <div className="mt-4 flex flex-wrap gap-3">
                <button onClick={() => setSavedAt(new Date().toLocaleString())} className="rounded-xl bg-amber-600 px-6 py-3 text-sm font-bold text-white hover:bg-amber-700 transition-colors duration-300">
                  Save Insight
                </button>
                <button onClick={handleShare} className="rounded-xl border-2 border-amber-300 bg-white px-6 py-3 text-sm font-bold text-amber-800 hover:bg-amber-50 transition-all duration-300">
                  Share
                </button>
                <button onClick={resetAll} className="rounded-xl border-2 border-rose-300 bg-white px-6 py-3 text-sm font-bold text-rose-700 hover:bg-rose-50 transition-all duration-300">
                  Reset
                </button>
              </div>
            </div>
      ...
      <Section title="Journey Progress" subtitle="Track your learning and continue your exploration">
        <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-stone-50 to-stone-100/50 p-8">
          <div className="grid gap-6 md:grid-cols-3 items-center">
            ...
            <div className="text-center">
              <div className="mb-3">
                <div className="text-3xl font-bold text-emerald-600 mb-1">{badges.length}</div>
      ...
      <div className="text-center">
        <p className="text-sm text-stone-500 italic">
          Replace placeholder texts with authenticated quotes and citations from your Sessions corpus.
        </p>
      </div>
    </div>
  )
